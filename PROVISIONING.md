# How to provision a new environment for fabermetrics
...
* Enter domains to secure (for example): `fabermetrics.com`
* Enter webroot path: `/home/deployer/apps/fabermetrics/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/fabermetrics.com/fullchain.pem /etc/ssl/fabermetrics.crt
ln -s /etc/letsencrypt/live/fabermetrics.com/privkey.pem /etc/ssl/fabermetrics.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/fabermetrics/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
47 0,12 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/fabermetrics/shared/public/
```
